...
def advanced_deterministic_extraction(text):
    ...
    for line in lines:
        ...
        if len(clean_line_check) > 4 and not any(k in line.lower() for k in ["invoice", "gstin", "tax", "date", "cell", "tele", "phone", "state"]):
            ...
            # Remove OCR line markers
            extracted_company = re.sub(
                r'<.*?>',
                ' ',
                extracted_company
            )
            ...
            for word in parts:
                if word.lower() in address_keywords:
                    break
    ...
    buyer_match_section = re.search(
            r"GSTIN\s*:\s*([A-Z0-9]+)\s*State\s*Code",
            clean_text,
            re.I
            )
...
def clean_and_normalize_digits(val):
    ...
    try:
        ...
        return None
# ---------- TEXT + GSTIN NORMALIZATION ----------

def remove_unwanted_spaces(value):
    """
    Removes extra spaces from OCR output
    """
    ...
    if isinstance(value, str):
        ...
        # Remove unwanted multiple spaces
        value = re.sub(r'\s+', ' ', value).strip()


        # Remove spaces ONLY from GSTIN format
        gst_pattern = re.match(
            r'^[0-9]{2}[A-Z0-9]{13}$',
            value.replace(" ", "").upper()
        )
...
def validate_and_correct_gstin(gstin):
    ...
    for i in range(0,2):
        if not chars[i].isdigit():
            chars[i] = digit_map.get(chars[i],"0")


    # PAN first 5 characters must be letters
    for i in range(2,7):
        if chars[i].isdigit():
            chars[i]="A"


    # PAN next 4 characters must be numbers
    for i in range(7,11):
        if chars[i].isalpha():
            chars[i]=digit_map.get(chars[i],"0")


    # Position 11 can be alphabet
    if not chars[11].isalpha():
        chars[11]="A"


    # Entity code
    # Entity code (13th character of GSTIN must be digit 1)
    chars[12] = "1"
...
def normalize_all_fields(data):
    ...
    """
    Apply space cleanup + GST correction
    """

    for key,value in data.items():

        if isinstance(value,str):
            data[key]=remove_unwanted_spaces(value)
    ...
    for field in gst_fields:
        ...
        if field in data:
            data[field]=validate_and_correct_gstin(
                data[field]
            )
...
def correct_amount_words_with_llm(amount_words):
    ...
    prompt = f"""
You are an invoice amount words grammar corrector.
...
"""
    ...
    try:
        ...
        # Remove LLaMA headings
        corrected = re.sub(
            r'^(Here\s+is.*?:|Corrected\s*:|Output\s*:|Answer\s*:)',
            '',
            corrected,
            flags=re.I
        ).strip()
        ...
        # If LLaMA returns quotes
        corrected = corrected.strip('"')


        # Ensure Only at end
        if not corrected.lower().endswith("only"):
            corrected += " Only"
...
try:
    ...
    # Remove markdown JSON wrapper if LLaMA adds it
    ...
    # -------- EXTRACT ONLY JSON FROM LLaMA OUTPUT --------
    # Removes extra text like:
    # "Note: I followed the strict behavioral rules..."
...
# NEW: remove spaces + GST validation
    parsed_json = normalize_all_fields(parsed_json)
    # Grammar correction for amount in words using LLaMA
    parsed_json["total_amount_in_words"] = correct_amount_words_with_llm(
    parsed_json.get("total_amount_in_words")
    )
